...
<div class="container mx-auto w-full text-center pt-32">
    ...
    <form method="POST" action="/products/<?php echo $product['ID'] ?>/update" class="flex flex-col items-center">
        <div class="w-1/2">
            <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Name:</label>
            <input type="text" name="name" id="name" value="<?php echo htmlspecialchars($product['name']) ?>" class="w-full border py-1 px-4 my-2 rounded-xl" required>
        </div>
        <div class="w-1/2 mt-4">
            <label for="description" class="block text-gray-700 text-sm font-bold mb-2">Description:</label>
            <textarea name="description" id="description" class="w-full border py-1 px-4 my-2 rounded-xl" required><?php echo htmlspecialchars($product['description']) ?></textarea>
        </div>
        <div class="w-1/2 mt-4">
        ...
        <div class="w-1/2 mt-4">
            ...
            <select name="supplier_id" id="supplier_id" class="w-full border py-1 px-4 my-2 rounded-xl" required>
                ...
                <?php foreach ($suppliers as $supplier): ?>
                    <option value="<?php echo $supplier['ID'] ?>"<?php echo $supplier['ID'] == $product['supplier_id'] ? 'selected' : '' ?>><?php echo htmlspecialchars($supplier['name']) ?></option>
                <?php endforeach; ?>
            </select>
        </div>
