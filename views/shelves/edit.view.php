...
<div class="w-full min-h-screen bg-gray-100 p-6">
    <h1 class="text-4xl font-extrabold bg-gradient-to-r from-blue-500 to-purple-900 text-transparent bg-clip-text mb-4">Edit Shelf #<?= $shelf['ID'] ?></h1>
    <form method="POST" action="/shelves/<?= $shelf['ID'] ?>/update" class="w-1/2 mx-auto">
        <div>
            <label for="name" class="block text-gray-700 text-sm font-bold mb-2">Shelf Name:</label>
            <input type="text" name="name" id="name" value="<?= htmlspecialchars($shelf['name']) ?>" class="w-full border py-1 px-4 my-2 rounded-xl" required>
        </div>
        <div class="mt-4">
            ...
            <select name="products[]" multiple class="w-full border py-1 px-4 my-2 rounded-xl h-40" size="5">
                <?php foreach ($allProducts as $product): ?>
                    <option value="<?= $product['ID'] ?>" <?= in_array($product['ID'], $currentProducts) ? 'selected' : '' ?>>
